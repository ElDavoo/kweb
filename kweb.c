/*                    Minimal Kiosk Browser 
   ...
   Copyright 2013-2016 Guenter Kreidl (Minimal Kiosk Browser)

   Version 1.7.8 using gtk+2 and webkit-1.0
*/
...
static gboolean autoconfig_enabled = TRUE;
static gboolean plugins_enabled = FALSE;
static gboolean mozplug = FALSE;
...
static gchar* command_flags = "-_+-zbhrqfpoklgtjneduwxycsamvi#?!.,";
...
static gchar* default_ua = "Mozilla/5.0 (X11; Linux armv6l) AppleWebKit/538.15 (KHTML, like Gecko) Version/8.0 Safari/538.15 kweb/1.7.8";
...
static void setPbutton(GtkWidget* window, gboolean pl)
{
	GtkWidget *Pbutton = g_object_get_data(G_OBJECT(window), "pbutton");
	gtk_toggle_tool_button_set_active(GTK_TOGGLE_TOOL_BUTTON( Pbutton),  pl);
}
...
static void togglePlugins(GtkWidget* item, WebKitWebView* webView)
{
	gboolean pl;
	WebKitWebSettings* settings = webkit_web_view_get_settings(WEBKIT_WEB_VIEW(webView));
	pl = gtk_toggle_tool_button_get_active(GTK_TOGGLE_TOOL_BUTTON(item));
	g_object_set(G_OBJECT(settings), "enable-plugins", pl , NULL);
}
...
{	gboolean pl;
	const char* uri = webkit_network_request_get_uri(request);
	WebKitWebSettings* settings = webkit_web_view_get_settings(WEBKIT_WEB_VIEW(webView));
	g_object_get(G_OBJECT(settings), "enable-plugins", &pl, NULL);
	...
	if (strstr(mimetype,"pdf") != NULL) {
        if ( ((strncmp(mimetype, "application/pdf", 15) == 0) || (strncmp(mimetype, "application/x-pdf", 17) == 0) || (strncmp(mimetype, "text/pdf", 8) == 0) || (strncmp(mimetype, "text/x-pdf", 10) == 0) ) && ((pl == FALSE) || (mozplug == FALSE))) {
	        int r = 0;
        	r = fork();
        	if (r == 0) {
...
void uri_changed (WebKitWebView  *web_view, WebKitWebFrame *frame, GtkWidget* entry)
{
	const gchar* uri = webkit_web_view_get_uri(web_view);
	const gchar* entry_str = gtk_entry_get_text(GTK_ENTRY(entry));
	if (strcmp(uri,entry_str) != 0) {
                                     ...
                                     gchar* arg)
{
	...
	if (parentWebView != NULL) {
		...
		g_object_get(G_OBJECT(psettings), "enable-plugins", &pl, NULL);
	...
	mitem = gtk_menu_item_new_with_label ("Quit Browser   Alt q");
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"document-open");
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Open file   Alt o", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"go-previous");
        gtk_toolbar_insert(GTK_TOOLBAR( toolbar ), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Back to last page   Alt b", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"process-stop");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Stop loading   Alt s", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"view-refresh");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Reload page   Alt r", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"go-home");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Open your homepage   Alt h", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"media-playback-start");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Play video from website   Alt p", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-in");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Zoom in by 10%   Alt +", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-out");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
	gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Zoom out by 10%   Alt -", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-original");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Reset zoom to 100%   Alt z", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Full Zoom");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Global, full zoom or text zoom only   Alt g,t", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Javascript");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Javascript or No Javascript   Alt j,n", NULL);
	...
	g_object_set_data( G_OBJECT(window), "jbutton", GTK_TOGGLE_TOOL_BUTTON( item ));
        ...
        item = gtk_toggle_tool_button_new();
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"preferences-other");
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Plugins");
        gtk_toolbar_insert(GTK_TOOLBAR( toolbar ), item, -1);
        gtk_toggle_tool_button_set_active(GTK_TOGGLE_TOOL_BUTTON( item ), pl);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Enable/Disable Plugins", NULL);
        g_signal_connect(G_OBJECT(item), "toggled", G_CALLBACK(togglePlugins), webView);
	g_object_set_data( G_OBJECT(window), "pbutton", GTK_TOGGLE_TOOL_BUTTON( item ));
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Cookies");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Enable/disable cookies   Alt e,d", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"DL-Manager");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," uGet or wget for downloads   Alt u,w", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Omxplayer");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Use omXplayer for media   Alt x,y", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Commands");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Activate/veto command execution   Alt a,v", NULL);
	...
	if ((kioskmode == FALSE) || ((kioskmode == TRUE) && (multiple_windows == TRUE))) {
    	...
    	g_signal_connect(webView,  "mime-type-policy-decision-requested", G_CALLBACK(mimePolicyDecision), window);
	g_signal_connect(webView, "scroll-event", G_CALLBACK(scroll_wheel), window);
	g_signal_connect(webView,  "notify::uri" , G_CALLBACK(uri_changed), uriEntry);
	if (kb_commands != "") {
...
int main( int argc, char* argv[] )
{
	...
	if (config != NULL) {
/*		if (strchr(config, '_') != NULL) {
			command_flags = g_strjoin(NULL,config,NULL); }
		else {
			command_flags = g_strjoin(NULL,config,"_",NULL);} */
		command_flags = g_strjoin(NULL,config,NULL);
		int l = strlen(config);
		int len;
		kb_commands = "";
	...
	if (g_file_test ("/usr/bin/mozplugger-controller",G_FILE_TEST_EXISTS)) {
		mozplug = TRUE;
	}
	kwebstartpage = g_strdup(starthere);
	if ((autoconfig_enabled == TRUE) && (commands_enabled == TRUE)) {
