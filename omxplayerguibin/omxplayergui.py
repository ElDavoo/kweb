...
import os,urllib,urllib2,sys,subprocess,threading,time,tkFileDialog,tkMessageBox,copy,imp,re
...
audioextensions = ['mp3','aac','flac','wav','wma','cda','ogg','ogm','ac3','ape','m4a']
streammode = 'video'
...
standalone = {'basedir':'','url':'','playlist':[],'open_settings':False,'root':None,'wcount':0,'layers':[]}
cmdoptions = []
server = None
...
def ustr_lower(ustr):
    try:
        tsrt = str(ustr.encode('utf-8'))
    except:
        tsrt = str(ustr)
    return tsrt.lower()

def ujoin(joiner,arr):
    res = u''
    if not arr or type(arr) != list:
        return res
    jp = unicode(joiner)
    for e in arr[0:len(arr)-2]:
        res += e + joiner
    res += arr[-1]
...
def check_server():
    ...
    if flag and cont == 'OK':
        ...
        return False
def check_ytdl_module():
    if os.path.exists(os.path.join(homedir,'youtube-dl')):
        return True
    else:
        try:
...
def start_server():
    svr = None
    pargs = ['ytdl_server.py']
    if preset_name:
        pargs.append('-t='+preset_name)
    try:
        svr = subprocess.Popen(pargs,stdin=subprocess.PIPE,stdout=file('/dev/null','wa'),stderr=file('/dev/null','wa'))
    except:
        pass
    return svr
...
class omxplayergui(tk.Frame):
    ...
    def __init__(self, master=None, playlist=[],mode='audio',autofinish=True,autoplay=True,volume=0,
                 ...
                 fontheight=14,fontname='SansSerif',maxlines=8,width=40, get_DAR = False, hide_controls = False,
                 standalone={'wcount':0,'layers':[]}, x_offset = 0, y_offset =  0):
        ...
        self.layer = 0
        if self.mode == 'AV':
            if self.standalone['wcount'] > 1:
                lc = 0
                for layer in self.standalone['layers']:
                    if lc <= layer:
                        lc = layer+1
                self.layer = lc
            self.standalone['layers'].append(self.layer)
        self.seekable = True
        ...
        smode = self.screenmode.get()
        if self.controls_hidden and self.mode == 'AV':
            self.hide_control()
        dummy = self.after(100, self.on_activate_first)
    ...
    def set_defaults(self):
        ...
        self.nextstring = '↓'
        self.vchdelay = 0.05
        self.videomodes = ['Mode:','full','refresh','auto','4:3','16:9','20:11','16:10','2.21:1','2.35:1','2.39:1','5:4']
    ...
    def set_title(self):
        if self.mode == 'audio':
            ...
            flags = ''
            name = "omxplayerGUI"
            if not self.dbuscontrol or not self.resize:
                flags += 'f'
            if self.get_DAR:
                flags += 'a'
            if self.layer > 0:
        ...
        if self.omxprocess:
            name = self.playlistwindow.get(self.songpointer)
            #name = self.namelist[self.songpointer]
        self.root.title(name+flags)
        ...
        if res:
            if 'DAR' in res:
                ...
                if ':' in dar:
                    ...
                    if len(darl) == 2:
                        ...
                        if gl:
                            ...
                            self.scalefactor = 16.0/9.0
            else:
                rlines = res.split('\n')
                video_found = False
                for rl in rlines:
                    if 'Stream' in rl and 'Video:' in rl and ',' in rl:
                        video_found = True
                        rln = rl.split(',',1)[1]
                        reg = re.compile('[0-9]+x[0-9]+')
                        regres = reg.search(rln)
                        if regres:
                            dar = regres.group()
                            darl = dar.split('x')
                            if len(darl) == 2:
                ...
                if not video_found:
                    self.currentmode = 'audio'
                    self.scalefactor = 16.0/9.0
        if res and 'Duration:' in res:
            pos1 = res.find('Duration:')
            pos2 = res.find(',',pos1)
            dar = res[pos1+10:pos2]
            if ':' in dar:
                darl = dar.split(':')
                if len(darl) == 3:
                    gl = True
                    ...
                    try:
                        ...
                        gl = False
                    if gl:
                        self.video_duration = int((hs*3600+ms*60+ss)*1000000)
                        self.seekable = True

    def get_duration(self,initial=True):
        self.seekable = False
        old_scalefactor = self.scalefactor
        dur = 0
        canseek = ''
        url = self.playlist[self.songpointer]
        if self.dbuscontrol:
            if initial and self.videomode.get() == 'auto':
                dur = self.video_duration
                canseek = str(self.seekable).lower()
            elif self.get_DAR:
                ...
                if initial:
                    time.sleep(2)
                count = 0
                while self.omxprocess and count < 15:
                    if dur == 0:
                    ...
                    if self.scalefactor == 1.0:
                        ...
                        if res:
                            ...
                            if self.scalefactor == 0.0:
                                self.scalefactor = 16.0/9.0
                                self.currentmode = 'audio'
                    if dur != 0 and canseek and self.scalefactor != 1.0:
                        break
                    count += 1
                    time.sleep(1)
            if self.omxprocess and dur != 0 and canseek == 'true':
                try:
                    ...
                    pass
            if self.omxprocess and (self.scalefactor != 1.0 or self.currentmode == 'audio'):
                try:
                    ...
                    if self.videomode.get() == 'auto' and self.scalefactor != old_scalefactor and self.currentmode == 'video':
                        self.trace_videomode()
                except:
                    pass
            if self.omxprocess:
                self.set_title()
    ...
    def screen_resize(self,height):
        ...
        if xpos + self.bgframe['width'] > self.root.winfo_screenwidth():
            newxpos =  self.root.winfo_screenwidth()-self.bgframe['width']
            if newxpos < 0:
                newxpos = 0
            xoffs = '+' + str(newxpos)
        if self.controls_hidden:
            ...
            ygoffs = self.vmodeoption.winfo_reqheight()+self.playbutton.winfo_reqheight()+66
        if ypos + self.bgframe['height'] + ygoffs > self.root.winfo_screenheight():
            newypos = self.root.winfo_screenheight() - height - ygoffs
            if newypos < 0:
                newypos = 0
            yoffs = '+' + str(newypos)
        ...
        geostr = str(top.winfo_reqwidth())+'x'+str(top.winfo_reqheight()) + xoffs + yoffs
        top.geometry(geostr)
...
#            f.write('\n'.join(self.playlist))
    ...
    def videowinsize(self, screenoffset=0):
        ...
        if vidh <= vh:
            ...
            varr = [str(xpos+screenoffset+self.x_offset),str(ypos+offset+self.y_offset),str(xpos+vw+screenoffset+self.x_offset),str(ypos+offset+vidh+self.y_offset)]
        else:
            vidw  = self.getwidth(vh,self.videomode.get())
            offset = int(round((vw-vidw)/2.0))
            varr = [str(xpos+offset+screenoffset+self.x_offset),str(ypos+self.y_offset),str(xpos+offset+vidw+screenoffset+self.x_offset),str(ypos+vh+self.y_offset)]
        return ' '.join(varr)
    ...
    def check_window(self,evt=None):
        if self.frozen:
            ...
            self.resize_after = self.root.after(50, self.resize_videoarea, 0)
...
#        st = event.state & 141
        ...
        if st == 0:
            ...
            if event.keysym in ['c','q']:
                ...
                if event.keysym in ['KP_Subtract','Next'] and self.layer > 0:
                    lc = self.layer -1
                    while lc in self.standalone['layers'] and lc > 0:
                        lc -= 1
                    if lc not in self.standalone['layers']:
                        if self.layer in self.standalone['layers']:
                            self.standalone['layers'].remove(self.layer)
                        self.standalone['layers'].append(lc)
                        self.layer = lc                        
                elif event.keysym in ['KP_Add','Prior']:
                    lc = self.layer +1
                    while lc in self.standalone['layers']:
                        lc += 1
                    if self.layer in self.standalone['layers']:
                        self.standalone['layers'].remove(self.layer)
                    self.standalone['layers'].append(lc)
                    self.layer = lc
                self.set_title()
            elif event.keysym == 'r' and self.mode == 'AV':
                if not self.omxprocess:
                    if self.get_DAR:
                        ...
                        self.get_DAR = True
                    self.set_title()
                elif self.omxprocess and self.currentmode == 'video' and not self.durwatcher:
                    if self.get_DAR:
                        self.get_DAR = False
                    else:
                        self.get_DAR = True
                    self.freeze_screen('auto')
                    self.durwatcher = threading.Timer(0,self.get_duration,args=[False])
        ...
        if not self.omxprocess:
            ...
            if self.mode == 'audio' or self.checkaudio(self.playlist[index]):
                ...
                if self.layer != 0:
                    ...
                    lopts = []
                topts = []
                if self.playlist[index].startswith('/'):
                    root, ext = os.path.splitext(self.playlist[index])
                    stpath = root + '.srt'
                    if os.path.exists(stpath):
                        topts = ['--subtitles',stpath]
                options = self.omxvideooptions + vopts + sopts + bopts + dbopts + lopts + topts
            ...
            if self.mode == 'AV':
                self.durwatcher = threading.Timer(0,self.get_duration,args=[self.get_DAR])
                self.durwatcher.daemon = True
                self.durwatcher.start()
    ...
    def on_close(self):
        if self.omxprocess:
            ...
            if self.mode == 'AV' and self.layer in self.standalone['layers']:
                self.standalone['layers'].remove(self.layer)
            self.root.destroy()
...
##        if self.currentmode == 'video':
    ...
    def createwidgets(self):
        ...
        if self.mode == 'AV':
            ...
            if mode == 'max':
                ...
                self.bgframe['height'] = self.root.winfo_screenheight()-self.vmodeoption.winfo_reqheight()-self.playbutton.winfo_reqheight()-66                
            elif mode == 'full':
                self.fullscreen()
                self.bgframe['width'] = self.root.winfo_screenwidth()
    ...
    def hide_control(self):
        ...
        dummy = self.root.after(200,self.reconfigure)
            ...
            if (url_extension in ['m3u','m3u8','pls'] or mode == 'pl' or 'mimetype' in ['audio/mpegurl','audio/x-mpegurl','audio/m3u','audio/x-scpls','audio/pls']) and mtflag:
                ...
                if playlist:
                    if audioonly and useVLC:
                        ...
                        if not standalone['root']:
                            player.mainloop()
##                    elif not audioonly and useVideoplayer:
                    elif useVideoplayer:
                        if not standalone['root']:
                            root = tk.Tk()
                        else:
                        ...
                        if not standalone['root']:
                            player.mainloop()
                    elif (audioonly and not omxplayer_in_terminal_for_audio) or (not audioonly and not omxplayer_in_terminal_for_video):
                        for indv in range(0,len(playlist)):
                            stopomx = False
                            namedoptions = []
                            if names and names[indv].startswith('omxoptions='):
                                nopt = names[indv].replace('omxoptions=','').split(';')
                                for opt in nopt:
                                    if not opt:
                                        continue
                                    if opt == "stop":
                                      ...
                                      namedoptions.append(opt)
                            if namedoptions:
                                options = namedoptions
                            elif cmdoptions:
                                options =[]
                                for copt in cmdoptions:
                                    if copt == 'stop':
                                        stopomx = True
                                    else:
                                        options.append(copt)
                                ...
                                options = omxoptions
                            if '--no-keys' not in options:
                                options.append('--no-keys')
                            if stopomx:
                                dummy = subprocess.call(['killall','omxplayer.bin'],stdout=file('/dev/null','wa'),stderr=file('/dev/null','wa'))
                            if stopomx and not namedoptions:
                                continue
                            else:
                                dummy = subprocess.call(['omxplayer']+options+[playlist[indv]]+['>', '/dev/null', '2>&1'])
                    else:
                        if audioonly:
                            options = omxaudiooptions
                        else:
                    ...
                    if url_extension in audioextensions or (streammode == 'audio' and not url_extension in videoextensions):
                        if useVLC:
                            ...
                            if omxplayer_in_terminal_for_audio:
                                ...
                                if '--no-keys' not in options:
                                    options.append('--no-keys')
                                pargs = ['omxplayer'] + options + [url]
                                dummy = subprocess.call(pargs,stdout=file('/dev/null','wa'),stderr=file('/dev/null','wa'))
    ...
    # web video section (HTML5 and all websites supported by youtube-dl)
            ...
            ytdl_paths = [homedir+'/youtube-dl/youtube_dl/__main__.py','/usr/local/bin/youtube-dl','/usr/bin/youtube-dl']
            if html5_first:
                tags = video_tag_extractor(url)
                if tags: #extract embedded html5 video
                    play_html5(tags)
##                elif 'https://www.youtube.com/results?search_query=' not in url and use_ytdl_server and check_server():
                    ...
                    yta = []
                    for yp in ytdl_paths:
                        if os.path.exists(yp):
                            yta = [yp, '-g', '-e','-i']+youtube_dl_options+[url]
                            break
                    if yta:
                ...
                res = ''
                flag = True
##                if 'https://www.youtube.com/results?search_query=' not in url and use_ytdl_server and check_server():
...
def usage():
    print 'usage: omxplayergui [--config=/path/to/config.py] [--preset=presetname] [--opts=omxoptions] [mode] [url] [mimetype]'
...
class entryApp(tk.Frame):
    def __init__(self, master,standalone,fontheight=12,fontname="SansSerif"):
        ...
        try:
            ...
            self.font = ('SansSerif',str(self.fontheight),'bold')
        self.root = master
        self.master.title('OmxplayerGUI Frontend 1.7.9')
        self.standalone = standalone
        self.help = None
        self.grid()
    ...
    def createWidgets(self):
        self.urlentry = tk.Entry(self,width=60,textvariable=self.url,font=self.font,takefocus=1,borderwidth=4)
    ...
    def showmanual(self):
        ...
        if not pdfprog:
            if os.path.exists('/usr/bin/evince'):
                ...
                pdfprog = 'xpdf'
            elif os.path.exists('/usr/bin/mupdf'):
                pdfprog = 'mupdf'
        if pdfprog:
        ...
        if url and url.startswith('?'):
            ...
            if '+' in searchstr:
                ...
                try:
                    ...
                    pass
            if searchstr:
                self.standalone['url'] = 'https://www.youtube.com/results?search_query='+searchstr
                if page:
...
if __name__ == '__main__':
    ...
    if '--config=' in checkstr or '--preset=' in checkstr or '--opts=' in checkstr:
        args = []
        for arg in sys.argv:
            if arg.startswith('--config='):
                ...
                if pname and os.path.exists('/usr/local/share/kweb/' + pname + '.preset'):
                    ...
                    preset_name = pname
            elif arg.startswith('--opts='):
                cmdoptions = arg.split('--opts=')[1].split(';')
            else:
                args.append(arg)
    else:
        args = sys.argv
    ...
    if len(args) > 2 and args[1] in ['av','-av','ytdl','-ytdl']:
        ...
        if use_ytdl_server and ytdl_server_host == 'localhost' and check_ytdl_module() and not check_server():
            server = start_server()
        while True:
            #useAudioplayer = True
            useVideoplayer = True
            ...
            if standalone['open_settings']:
                try:
                    kweb = subprocess.Popen(['gksudo','kweb -_ISAMCU0+-zbhrqfpoklgtjneduwxyavcsmi#?!., file:///usr/local/share/kweb/kweb_about_s.html'])
                ...
                if settings and os.path.exists(settings):
                    try:
                        ...
                        pass
                    if server and not use_ytdl_server and ytdl_server_host == 'localhost':
                        stop_server()
                    if use_ytdl_server and ytdl_server_host == 'localhost' and check_ytdl_module() and not check_server():
                        server = start_server()
            else:
